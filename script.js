  ...
  document.getElementById('Submit-register').addEventListener('click', function () {
    ...
    if (password !== rePassword) {
      showGlassAlert();
        document.getElementById('alert').innerText='Conform password do not match';
      return;
    }
    ...
    usersRef.orderByChild("email").equalTo(email).once("value", function (emailSnap) {
      if (emailSnap.exists()) {
        showGlassAlert();
        document.getElementById('alert').innerText='Email already registered';
        return;
      }
      ...
      usersRef.orderByChild("username").equalTo(username).once("value", function (userSnap) {
        if (userSnap.exists()) {
          showGlassAlert();
        document.getElementById('alert').innerText='Username is already taken';
          return;
        }
        ...
        usersRef.push({
          ...
          showGlassAlert();
        document.getElementById('alert').innerText='Email registered';
